...
import { Terminal, Code2, Sparkles, FolderArchive, FileCode, PlusCircle } from 'lucide-react';
...
export default function SecurityToolsSection({ refreshTrigger }) {
  const [toolsList, setToolsList] = useState(defaultTools);
  ...
  useEffect(() => {
    fetchTools();
  }, [refreshTrigger, fetchTools]);

  const defaultCategories = ['All', 'Web VAPT Scanner', 'Mobile Security Utility', 'Network Infrastructure Tool', 'Exploit PoC'];
  const categories = Array.from(new Set(['All', ...defaultCategories.slice(1), ...toolsList.map(t => t.category).filter(Boolean)]));
  ...
  return (
    <section id="tools" style={{ padding: '60px 24px 30px 24px', maxWidth: '1280px', margin: '0 auto' }}>
      ...
      {filteredTools.length > 0 ? (
            ...
            >
              <div>
                ...
                <div style={{ fontSize: '0.85rem', color: 'var(--accent-primary)', fontWeight: 600, fontFamily: 'var(--font-mono)', marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <Code2 size={14} /> Language: {tool.language || 'Python'}
                </div>
        ...
        /* Friendly Empty State if filter yields 0 cards */
        <div style={{
          textAlign: 'center',
          padding: '40px 20px',
          background: 'var(--bg-card-solid)',
          borderRadius: '20px',
          border: '1px border var(--border-light)',
          margin: '10px 0'
        }}>
          <Sparkles size={32} style={{ color: '#4F46E5', marginBottom: '12px' }} />
          <h4 style={{ fontSize: '1.2rem', fontWeight: 700, margin: '0 0 8px 0', color: 'var(--text-primary)' }}>
            No tools in "{selectedCategory}" yet!
          </h4>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '16px' }}>
            Be the first visitor to contribute a tool to this category.
          </p>
          <a href="#upload" className="btn-primary" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '8px 16px', fontSize: '0.85rem', textDecoration: 'none' }}>
            <PlusCircle size={15} /> Upload Tool in this Category
          </a>
        </div>
      )}
